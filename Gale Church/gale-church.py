...
def main(corpusx, corpusy, outfile_x, outfile_y, outfile_3, mean=1.0, variance=6.8, bc = BEAD_COSTS):
  if mean == "gacha":
  ...
  for src,trg in zip(readFile(corpusx),readFile(corpusy)):
    ...
    for (sentence_x, sentence_y) in align(src[0], trg[0], mean, variance, bc):
      ...
      out_x.append(sentence_x)
      out_y.append(sentence_y)
      b = sentence_x + "\t" + sentence_y
      out_3.append(b)
      
    with open(outfile_x, 'w', encoding = "utf-8") as myfile:
        newdata = '\n'.join(out_x)
        myfile.write(newdata)
        
    with open(outfile_y, 'w', encoding = "utf-8") as myfile:
        newdata = '\n'.join(out_y)
        myfile.write(newdata)
        
    with open(outfile_3, 'w', encoding = "utf-8") as myfile:
        newdata = '\n'.join(out_3)
        myfile.write(newdata)
      

if __name__ == '__main__':
    #main("churchenglish200c.txt", "churchpolish200c.txt","GCeng.txt","GCpol.txt","GCboth.txt")
    
    main("Sample_source_corpus.txt", "Sample_target_corpus.txt", "GCsource.txt", "GCtarget.txt", "GCsampleboth.txt")
